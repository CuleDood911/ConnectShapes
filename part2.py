...
def gravity(playing_field,x,y):
    for i in range(x * y - 1, -1, -1):
        if playing_field[i][2] == 0 and i >= x:
            playing_field[i][2] = playing_field[i - x][2]
            playing_field[i - x][2] = 0
...
def display(playing_field,x,y,lent,totalscore,finalscore,clickcoords,clicklocal,attempts,level,background,win):
    ...
    if attempts == 0 and totalscore < finalscore:
        while stddraw.mousePressed() == False:
            ...
            stddraw.setFontSize(20)
            stddraw.setPenColor(stddraw.RED)
            stddraw.text(lent * x - lent,-( lent * y),'YOU LOSE, DOUBLE CLICK TO EXIT')
            stddraw.show(0)
    ...
    if totalscore >= finalscore:
        while stddraw.mousePressed() == False:
            ...
            stddraw.setFontSize(20)
            stddraw.setPenColor(stddraw.GREEN)
            stddraw.text(lent * x - lent,-( lent * y),'YOU WIN, DOUBLE CLICK TO PROCEED')
            stddraw.show(0)
            if stddraw.mousePressed() == True:
                win = False
                return win
